...
class SVGService:
    ...
    def fix_font_urls_in_svg(svg_content: str, svg_file_id: int, db: Session, request: Request) -> str:
        ...
        def replace_font_url(match):
            ...
            font_file = db.query(FontFile).filter(
                FontFile.svg_file_id == svg_file_id,
                FontFile.filename == font_filename
            ).first()
    ...
    def save_svg_file(file_content: bytes, filename: str, db: Session) -> Dict[str, Any]:
        ...
        return {
            "file_id": db_svg.id,
            "filename": filename,
            "required_fonts": font_references
        }
    ...
    def process_zip_in_background(task_id: str, zip_content: bytes, filename: str):
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                all_font_files = list(temp_path.rglob("*.woff")) + list(temp_path.rglob("*.woff2")) + list(temp_path.rglob("*.ttf")) + list(temp_path.rglob("*.otf"))
                font_files = [f for f in all_font_files if is_valid_font_file(f)]
                
                if not svg_files:
                    skipped_count = len(all_svg_files) - len(svg_files)
                    message = f"Error: No valid SVG files found (skipped {skipped_count} system files)" if skipped_count > 0 else "Error: No SVG files found in ZIP"
                    SVGService.update_progress(task_id, 100, 100, message)
                    return
                
                SVGService.update_progress(task_id, 15, 100, f"Found {len(svg_files)} SVG files and {len(font_files)} font files")
                ...
                for i, svg_file in enumerate(svg_files):
                    ...
                    SVGService.update_progress(task_id, svg_progress, 100, f"Processing SVG {i+1}/{len(svg_files)}: {svg_file.name}")
                ...
                upload_progress[task_id] = {
                    ...
                    "completed": True,
                    "result": {
                        "processed_svgs": processed_svgs,
                        "unmatched_fonts": unmatched_fonts
                    }
                }
