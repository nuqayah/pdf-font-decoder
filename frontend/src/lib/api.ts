...
class APIClient {
  ...
  async getSvgs(
    page: number = 1,
    limit: number = 50
  ): Promise<SVGListResponse> {
    const response = await fetch(
      `${this.baseUrl}/svgs?page=${page}&limit=${limit}`
    );
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  }
  ...
  async getSourceOfTruthSvg(
    svgFileId: number
  ): Promise<{ source_of_truth_content: string }> {
    const response = await fetch(
      `${this.baseUrl}/svg/${svgFileId}/source-of-truth`
    );
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  }
